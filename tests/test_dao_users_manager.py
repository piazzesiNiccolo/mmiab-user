...
class TestUserManager:
    ...
    @pytest.mark.parametrize("email, phone",[
        ("email@email.com","38217192937"),
        ("email1@email1.com","1234567890")
    ])
    ...
    @pytest.mark.parametrize("keyword, length",[
        ("Faella", 2),
        ("Lorenzo", 1),
        ("Marco", 2),
        ("", 2),
    ...
    def test_filter_by_keword(self, users, keyword, length):
        assert len(UserManager.filter_users_by_keyword(users, key_word=keyword)) == length
